...
let grouperClient = null;
function initializeGrouperClient() {
    if (!grouperClient) {
        const config = {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    switch (request.params.name) {
        ...
        case 'grouper_get_group': {
            const { groupName } = request.params.arguments;
            try {
        ...
        case 'grouper_create_group': {
            const { name, displayName, description } = request.params.arguments;
            try {
        ...
        case 'grouper_update_group': {
            const { groupName, displayName, description } = request.params.arguments;
            try {
                const updates = {};
        ...
        case 'grouper_add_member': {
            const { groupName, subjectId, subjectSourceId, subjectIdentifier } = request.params.arguments;
            try {
        ...
        case 'grouper_assign_attribute': {
            const { groupName, attributeName, value } = request.params.arguments;
            try {
